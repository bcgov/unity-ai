...
docker tag unity-ai-reporting $OC_REGISTRY/$OC_TARGET_PROJECT/dev2-unity-ai-reporting
docker push $OC_REGISTRY/$OC_TARGET_PROJECT/dev2-unity-ai-reporting

# Debugging commands forced restart process for frontend StatefulSet
# Scale down
oc -n d18498-dev scale statefulset/dev2-unity-ai-reporting --replicas=0
# Restart the frontend StatefulSet
oc -n d18498-dev rollout restart statefulset dev2-unity-ai-reporting
# Scale up to desired replicas 
oc -n d18498-dev scale statefulset/dev2-unity-ai-reporting --replicas=1